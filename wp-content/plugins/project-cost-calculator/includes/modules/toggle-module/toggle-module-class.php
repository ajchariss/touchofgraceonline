<?php
if( !class_exists( 'PRC_ToggleModule' ) ){
    class PRC_ToggleModule extends PRC_module {

        function __construct(){

            $this->setModuleID('toggle');
            $this->setModuleName("Toggle");
            $this->setModuleIconUrl(plugin_dir_url(__FILE__)."icon/toggle.svg");
            ...
            $this->setSettingField( "active_name", array(
                "name" => "Active Label",
                "element" => "input",
                "attributes" => array(
                    "name" => "active_name",
            ...
            $this->setSettingField( "deactive_name", array(
                "name" => "Inactive Label",
                "element" => "input",
                "attributes" => array(
                    "name" => "deactive_name",
            ...
            $this->setSettingField( "default_state", array(
                ...
                "attributes" => array(
                    ...
                    "id" => "simple_input_setting_field",
                    "type" => "select",
                    "list" => array( "activated" => "Activated", "deactivated" => "Deactivated" ),
                )
            ) );
        ...
        public function render($data){
            ob_start();
            $file = plugin_dir_path( __FILE__ )."template/toggle.php";
            if( file_exists( $file ) ){
                include $file;
            }
            return ob_get_clean();
...
PRC_fields_module::register_module(new PRC_ToggleModule());
?>