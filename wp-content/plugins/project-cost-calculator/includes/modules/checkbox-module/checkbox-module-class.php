<?php
if( !class_exists( 'PRC_CheckboxModule' ) ){

    class PRC_CheckboxModule extends PRC_module {

        function __construct(){

            $this->setModuleID('checkbox');
            $this->setModuleName("Checkbox");
            $this->setModuleIconUrl(plugin_dir_url(__FILE__)."icon/checkbox-icon.svg");
        ...
        public function render( $data ){
            ob_start();
            $file = plugin_dir_path( __FILE__ )."template/checkbox.php";
            if( file_exists( $file ) ){
                include $file;
            }
            return ob_get_clean();
...
PRC_fields_module::register_module(new PRC_CheckboxModule());
?>