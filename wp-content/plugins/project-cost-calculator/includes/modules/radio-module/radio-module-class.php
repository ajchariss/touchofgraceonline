<?php
if( !class_exists( 'PRC_RadioModule' ) ){
    class PRC_RadioModule extends PRC_module {
        

        function __construct(){

            $this->setModuleID('radio');
            $this->setModuleName("Radio");
            $this->setModuleIconUrl(plugin_dir_url(__FILE__)."icon/radio.svg");
        ...
        public function render( $data ){
            ob_start();
            $file = plugin_dir_path( __FILE__ )."template/radio.php";
            if( file_exists( $file ) ){
                include $file;
            }
            return ob_get_clean();
...
PRC_fields_module::register_module(new PRC_RadioModule());
?>