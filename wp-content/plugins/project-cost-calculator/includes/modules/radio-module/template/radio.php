        ...
        foreach ($values['option']['labels'] as $key => $value) { 
            ?>
                <div class="pro_rate_cal_field_radio_item">
                    <input type="radio" id="<?php echo esc_attr($values['label_name']).$key; ?>"  value="<?php echo esc_attr( $values['option']['values'][$key] ); ?>" name="<?php echo esc_attr( str_replace( " ", "-", $values['label_name'] ) ); ?>"  <?php 
