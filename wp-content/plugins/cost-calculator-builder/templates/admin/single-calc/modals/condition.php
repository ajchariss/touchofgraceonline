				...
				<div class="condition">
					...
					<div class="select-with-label" v-if="!['in', 'not in', 'contains'].includes(additionalCondition.condition)">
						...
						<select v-model="additionalCondition.key" v-if="$store.getters.getConditionData.type === 'select' && !['>=', '<=', '== & distance', '<= & distance', '>= & distance', '!= & distance'].includes(additionalCondition.condition)">
							...
							<option v-for="(item, key) in $store.getters.getFieldOptionsByFieldId($store.getters.getConditionData.optionFrom)" :value="key" v-if="$store.getters.getConditionData.optionFrom.includes('geolocation')">
								{{ item.label }}
							</option>
							<option v-for="(item, key) in $store.getters.getConditionOptions" :value="key" v-else>
								{{ item.optionText }}
							</option>
						</select>
